...
where <img src="https://latex.codecogs.com/png.latex?\mathbbm{1}_i(\mathbf{x})" /> is the indicator function on set $i$ , $A_l$ and $A_r$ denote the support of the left and right lungs respectively. These integrals can be numerically calculated using masks for left and right lungs (row 3) obtained using the AI-based tool for lung segmentation in [4]. For the images shown, values are: <img src="https://render.githubusercontent.com/render/math?math={ q_l= 327 }"> and  <img src="https://render.githubusercontent.com/render/math?math={ q_r=60 }"> for the first column (left dominant) and
